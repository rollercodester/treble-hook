...
describe('usePubSub', () => {
  ...
  it(`should return given default state if no publish has been called for topic`, () => {
    ...
    expect(subscriber1.current[STATE_VALUE_INDEX]).toBe(TEST_TOPIC_1_DEFAULT_STATE)
  ...
  it(`should return current publish state instead of given default state if a publish
    ...
    act(() => {
      subscriber1.current[PUBLISH_INDEX](TEST_TOPIC_1_PUBLISH_STATE_1)
    })

    const { result: subscriber2 } = renderHook(() => usePubSub<string>(TEST_TOPIC_1, TEST_TOPIC_2_DEFAULT_STATE))

    expect(subscriber2.current[STATE_VALUE_INDEX]).toBe(TEST_TOPIC_1_PUBLISH_STATE_1)
  ...
  it(`should publish state to all subscribers of published topic`, () => {
    ...
    expect(subscriber1.current[STATE_VALUE_INDEX]).toBe(TEST_TOPIC_1_PUBLISH_STATE_1)
    expect(subscriber2.current[STATE_VALUE_INDEX]).toBe(TEST_TOPIC_1_PUBLISH_STATE_1)
  ...
  it(`should NOT publish state to subscribers that not subscribed to published topic`, () => {
    ...
    expect(subscriber1.current[STATE_VALUE_INDEX]).toBe(TEST_TOPIC_1_PUBLISH_STATE_1)
    expect(subscriber2.current[STATE_VALUE_INDEX]).toBe(TEST_TOPIC_2_DEFAULT_STATE)
  ...
  it(`should no longer publish to a subscription when the subscriber requests to be unsubscribed`, () => {
    ...
    act(() => {
      subscriber2.current[UNSUBSCRIBE_INDEX]()
      subscriber1.current[PUBLISH_INDEX](TEST_TOPIC_1_PUBLISH_STATE_2)
    })

    expect(subscriber1.current[STATE_VALUE_INDEX]).toBe(TEST_TOPIC_1_PUBLISH_STATE_2)
    expect(subscriber2.current[STATE_VALUE_INDEX]).toBe(TEST_TOPIC_1_PUBLISH_STATE_1)
